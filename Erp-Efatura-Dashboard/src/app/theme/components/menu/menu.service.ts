...
/*import { verticalMenuItemsx } from './menu';
import { horizontalMenuItemsx } from './menu';*/
...
export class MenuService {
  ...
  public getVerticalMenuItems():Array<Menu> {
    ...
    return this.getHorizontalMenuItems();
  }

  public getHorizontalMenuItems() : Array<Menu> {
    const user: User = JSON.parse(localStorage.getItem('currentUser'));
    const horizontalMenuItems = [new Menu (1, 'Anasayfa', '/pages/dashboard', null, 'tachometer', null, false, 0)];
    horizontalMenuItems.push((new Menu (2, 'Organizasyon', null, null, 'industry', null, true, 0)));
    if (user['tenantUsers']['userRole']['tenant'] !== 'x' && 
    user['tenantUsers']['tenant']['tenantType'] !== 'CUSTOMER' ) {
      horizontalMenuItems.push(new Menu (23, 'Organizasyonlar', '/pages/tenants', null, 'industry', null, false, 2));
    }
    if (user.tenantUsers.userRole.user !== 'x') {
      horizontalMenuItems.push((new Menu (24, 'Kullanıcı', '/pages/users', null, 'user', null, false, 2)));
    }
    if (user['tenantUsers']['userRole']['customer'] !== 'x') {
      horizontalMenuItems.push(new Menu (4, 'Cari', '/pages/customers', null, 'id-card-o', null, false, 0));
    }
    horizontalMenuItems.push(new Menu (5, 'Kasa', null, null, 'briefcase', null, true, 0));
    if (user['tenantUsers']['userRole']['account'] !== 'x') {
      horizontalMenuItems.push(new Menu (51, 'Hesaplar', '/pages/accounts', null, 'credit-card', null, false, 5));
    }
    // --------------
    if (user['tenantUsers']['userRole']['account'] !== 'x') {
      horizontalMenuItems.push(new Menu (52, 'Masraf Kalemleri', '/pages/spending-types', null, 'list-ol', null, false, 5));
    }
    // ----------
    if (user['tenantUsers']['userRole']['item'] !== 'x') {
      horizontalMenuItems.push(new Menu (6, 'Stok', null, null, 'cubes', null, true, 0));
    }
    if (user['tenantUsers']['userRole']['item'] !== 'x') {
      horizontalMenuItems.push(new Menu (71, 'Depo', '/pages/stocks/storage', null, 'archive', null, false, 6));
    }
    if (user['tenantUsers']['userRole']['item'] !== 'x') {
      horizontalMenuItems.push(new Menu (7, 'Kategori', '/pages/stocks/categories', null, 'list', null, false, 6));
    }
    if (user['tenantUsers']['userRole']['item'] !== 'x') {
      horizontalMenuItems.push(new Menu (8, 'Ürün', '/pages/stocks/items', null, 'gift', null, false, 6));
    }
    if (user['tenantUsers']['userRole']['invoice'] !== 'x') {
      horizontalMenuItems.push(new Menu (9, 'Fatura', null, null, 'calculator', null, true, 0));
      /*
      horizontalMenuItems.push(new Menu (91, 'Yeni Giden Fatura', '/pages/allinvoices/invoicesOutbox/add', null, 'envelope-square', null, false, 9));
      horizontalMenuItems.push(new Menu (92, 'Yeni Gelen Fatura', '/pages/allinvoices/invoicesInbox/add', null, 'envelope-square', null, false, 9));
      */
      horizontalMenuItems.push(new Menu (93, 'Gelen Kutusu', '/pages/allinvoices/invoicesInbox', null, 'envelope-open', null, false, 9));
      horizontalMenuItems.push(new Menu (94, 'Giden Kutusu', '/pages/allinvoices/invoicesOutbox', null, 'envelope', null, false, 9));
    }
    if (user['tenantUsers']['userRole']['isEinvoiceAdmin']) {
      horizontalMenuItems.push(new Menu (11, 'Efatura Yönetim', null, null, 'file', null, true, 2));
      if (user['tenantUsers']['tenant']['tenantType'] === 'SUPERADMIN') {
        horizontalMenuItems.push(new Menu (111, 'Admin', '/pages/einvoice-admin/admin', null, 'user-secret', null, false, 11));
      }
      horizontalMenuItems.push(new Menu (112, 'Mükellef', '/pages/einvoice-admin/user', null, 'user', null, false, 11));
      horizontalMenuItems.push(new Menu (113, 'Kontör', '/pages/einvoice-admin/credit', null, 'credit-card', null, false, 11));
      if (user['tenantUsers']['tenant']['tenantType'] === 'SUPERADMIN') {
        horizontalMenuItems.push(new Menu (114, 'E-Arşiv', '/pages/einvoice-admin/earchive', null, 'archive', null, false, 11));
      }
    }
    if (user['tenantUsers']['tenant']['tenantType'] === 'SUPERADMIN') {
      horizontalMenuItems.push(new Menu (12, 'Üye Yönetimi', '/pages/manage', null, 'share-alt', null, false, 2));
    }
    if (user['tenantUsers']['userRole']['isTenantAdmin']) {
      horizontalMenuItems.push(new Menu (13, 'Ayarlar', '/pages/options', null, 'wrench', null, false, 0));
    }
