...
export class DefaultModalAccountActivities implements OnInit {
  ...
  private _service: NotificationsService) {
    this.accountActivities = new Array<AccountActivity>();
  ...
  getSpendingType() {
    ...
    body.set('accountId', `${this.id}`);
    body.set('limit', `${this.limit}`);
    body.set('offset', `${this.offset * this.limit}`);
    this.newService.fetchPost(`${GlobalTexts.rest_url}sessions/account/getActivities`, body, this.options).
    subscribe(
    (data) => {
      if (!data.error) {
